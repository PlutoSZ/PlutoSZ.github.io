(() => {
  ...
  (() => {
    ...
    const scriptRequests = /* @__PURE__ */ new Map();
    const repositoryRequests = /* @__PURE__ */ new Map();
    let initialization;
    const loadScript = (url) => {
      if (scriptRequests.has(url)) return scriptRequests.get(url);
      const request = window.Solitude?.getScript ? window.Solitude.getScript(url) : new Promise((resolve, reject) => {
    ...
    const requestRepository = (endpoint) => {
      ...
      const request = fetch(endpoint, { headers: { Accept: "application/json" } }).then((response) => {
    ...
    const init = () => {
      ...
      const request = Promise.allSettled([
        ...
        if (initialization === request) initialization = void 0;
      });
